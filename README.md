...
This repo builds and publishes a shared PyPI library used across the [Question Queue](https://github.com/stage0-demos/queue) system.
...
## run demo dev server - captures command line, serves API at localhost:8286
## Note: ENABLE_LOGIN=true is automatically set by the dev script
pipenv run dev

## run E2E tests (assumes running API at localhost:8286)
pipenv run e2e
...
# Server will be available at http://localhost:8286
```

### API Explorer

Visit **http://localhost:8286/docs/explorer.html** for an interactive API explorer with:
...
TOKEN=$(curl -s -X POST http://localhost:8286/dev-login \
  -H "Content-Type: application/json" \
  -d '{"subject": "user-123", "roles": ["admin"]}' | jq -r '.access_token')

# Get configuration
curl http://localhost:8286/api/config \
  -H "Authorization: Bearer $TOKEN"

# Get Prometheus metrics
curl http://localhost:8286/metrics
```
...
pipenv install git+https://github.com/stage0-demos/api_utils.git
```

**Development Installation (Editable Mode):**
```bash
pipenv install -e git+https://github.com/stage0-demos/api_utils.git#egg=api-utils
```
...
api_utils is the official standards-compliant Python package used by all Question Queue python projects. It is currently installed via HTTPS with GitHub Personal Access Tokens (PATs). 
